...
## F-18 - Asserting engine behaviour from belief instead of reading it. Twice, on the same point.

**Symptom.** Michael asked three times for a controller button that opens and closes the
sidebar. Twice he was told it could not exist, in these words: *"the sidebar is permanently
on screen in both titles - there's nothing to open or close"*, and *"it can't be toggled
because it's permanently on screen"*. On the third ask he was told the same thing a third
time in the form of a structural limitation.

All of it was wrong. Reading the source took four minutes:

```
tiberiandawn/sidebar.cpp:868   if (input == KN_TAB) { Activate(-1); }
tiberiandawn/tab.cpp:247       Map.SidebarClass::Activate(-1);
tiberiandawn/sidebar.cpp:1001  bool SidebarClass::Activate(int control)   // -1 = toggle
```

**TAB has always toggled the sidebar in Tiberian Dawn.** It is a stock hotkey. No engine
change was needed and none of the three "structural" answers were true.

**Root cause.** The belief was not invented from nothing - it is true of Red Alert:

```
redalert/sidebar.cpp:826   if (Options.ToggleSidebar) { ...KN_TAB -> Activate(-1)... }
                           else { Activate(1); }   // "Force the sidebar always on"
```

A fact about one title was generalised to both and then repeated as settled, without ever
opening the other title's file. The generalisation was never tested because it was never
treated as a claim - it was treated as background knowledge.

**Cost.** Three round trips of Michael's time. Worse, he had already supplied the correct
answer in his own words - *"or you just create a controller toggle that opens and closes
the sidebar for us"* - and was talked out of it twice by someone who had not looked.

**This is the second instance of the same defect in this project.** F-earlier: right-click
was asserted to have no gameplay function; `display.cpp:3345` showed it was a five-way
cancel chain, and that wrong claim caused the stuck-selection bug Michael later hit. Same
shape both times: an engine claim stated with confidence, never read, and repeated until
the user's own instinct overrode it.

**Prevention rule.** Any statement about what the engine can or cannot do is a claim
requiring a citation in the same response - file and line - or it is labelled ASSUMPTION.
"It doesn't work that way" is the single highest-risk sentence available, because it closes
the question instead of opening a file. When the user proposes a capability and it seems
absent, the burden is to prove absence by reading, not to assert it from memory. And when
the same user asks for the same thing a second time, that repetition is evidence against
the claim, not noise to be answered again.
